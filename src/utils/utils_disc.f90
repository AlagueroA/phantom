...
subroutine bindisc_analysis(xyzh,vxyz,npart,pmass,time,nbin,rmin,rmax,G,M_star,&
                     tilt,tilt_acc,twist,twistprev,psi,H,bin,h_smooth,sigma,unitlx,unitly,unitlz,&
                     Lx,Ly,Lz,Lx_mean,Ly_mean,Lz_mean,&
                     ecc,ninbin,assume_Ltot_is_same_as_zaxis,xyzmh_ptmass,vxyz_ptmass,nptmass,&
                     incl,Omega,omega_,ref_is_only_sinks,sinks_CoM,sinks_L)
 ...
 integer, intent(in)              :: nbin,npart
 real, intent(in)                 :: rmin,rmax,G,M_star
 logical, intent(in)              :: assume_Ltot_is_same_as_zaxis,ref_is_only_sinks
 integer, intent(in)              :: sinks_CoM(:),sinks_L(:)
 real, optional, intent(inout)    :: xyzmh_ptmass(:,:),vxyz_ptmass(:,:)
 integer, optional, intent(inout) :: nptmass
 real, intent(out)                :: Lx(nbin),Ly(nbin),Lz(nbin),bin(nbin),unitly(nbin)
 real, intent(out)                :: Lx_mean(nbin),Ly_mean(nbin),Lz_mean(nbin)
 real, intent(out)                :: incl(nbin), Omega(nbin), omega_(nbin)
 real, intent(out)                :: tilt(nbin),tilt_acc(nbin),twistprev(nbin)
 real, intent(out)                :: psi(nbin),H(nbin),ecc(nbin),unitlz(nbin)
 real, intent(out)                :: sigma(nbin),h_smooth(nbin),unitlx(nbin)
 integer, intent(out)             :: ninbin(nbin)
 ...
 real                             :: angtot,Ltot
 real                             :: rsphi,rcyli,area,Ei,mu,term,ecci
 real                             :: Li(3),Lsi(3),xi(3),vi(3),Limag,dtwist
 real                             :: ecc_veci(3),ecc_vec_x(nbin),ecc_vec_y(nbin),ecc_vec_z(nbin),ecc_vec(3,nbin)
 real                             :: node_vec_x(nbin),node_vec_y(nbin),node_vec_z(nbin),node_vec(3,nbin)
 real                             :: hyp(nbin)
 real                             :: psi_x,psi_y,psi_z,tp(nbin)
 real                             :: L_tot(3),L_tot_mag,temp(3),temp_mag
 real                             :: rotate_about_z,rotate_about_y
...
! Move everything so that the centre of mass is at the origin
! and run everything in CoM frame
! NB: the CoM is not necessarily the one of the whole system according to ref_is_only_sinks and sinks_index
 if (nptmass > 0) then
    call reset_centreofmass(npart,xyzh,vxyz,nptmass,xyzmh_ptmass,vxyz_ptmass,only_sinks=ref_is_only_sinks,sinks_index=sinks_CoM)
 endif
...
! Loop over particles putting properties into the correct bin
 do i = 1,npart
    ...
    if (xyzh(4,i)  >  tiny(xyzh)) then ! IF ACTIVE
       ...
       Lsi(1) = xi(2)*vi(3) - xi(3)*vi(2)
       Lsi(2) = xi(3)*vi(1) - xi(1)*vi(3)
       Lsi(3) = xi(1)*vi(2) - xi(2)*vi(1)
       
       ecc_veci(1) = (vi(2)*Lsi(3) - vi(3)*Lsi(2))/(G*M_star) - xi(1)/rsphi
       ecc_veci(2) = (vi(3)*Lsi(1) - vi(1)*Lsi(3))/(G*M_star) - xi(2)/rsphi
       ecc_veci(3) = (vi(1)*Lsi(2) - vi(2)*Lsi(1))/(G*M_star) - xi(3)/rsphi
       ...
       ecc_vec_x(ii) = ecc_vec_x(ii) + ecc_veci(1)
       ecc_vec_y(ii) = ecc_vec_y(ii) + ecc_veci(2)
       ecc_vec_z(ii) = ecc_vec_z(ii) + ecc_veci(3)
       ecc(ii) = ecc(ii) + ecci
...
! Convert total angular momentum into a unit vector, and average h_smooth
 do i = 1,nbin
    ...
    if (ninbin(i) > 0) then
       ...
       Lx_mean(i) = Lx(i)/ninbin(i)
       Ly_mean(i) = Ly(i)/ninbin(i)
       Lz_mean(i) = Lz(i)/ninbin(i)

       h_smooth(i) = h_smooth(i)/ninbin(i)
       ecc_vec_x(i) = ecc_vec_x(i)/ninbin(i)
       ecc_vec_y(i) = ecc_vec_y(i)/ninbin(i)
       ecc_vec_z(i) = ecc_vec_z(i)/ninbin(i)

       ecc_vec(1,i) = ecc_vec_x(i)
       ecc_vec(2,i) = ecc_vec_y(i)
       ecc_vec(3,i) = ecc_vec_z(i)
...
! Now loop over rings to calculate required quantities
 do i = 1, nbin
    if (ninbin(i)==0 .or. ninbin(i)==1) then
       ...
       h_smooth(i) = h_smooth(i)/H(i)
       incl(i) = acos(unitlz(i))
       
       node_vec_x(i) = -unitly(i)
       node_vec_y(i) = unitlx(i)
       node_vec_z(i) = 0.0

       node_vec(1,i) = node_vec_x(i)
       node_vec(2,i) = node_vec_y(i)
       node_vec(3,i) = node_vec_z(i)
       
       if (node_vec_y(i)>=0) then ; Omega(i) = acos(node_vec_x(i)/sqrt(unitlx(i)**2+unitly(i)**2))
       else ; Omega(i) = 2*pi-acos(node_vec_x(i)/sqrt(unitlx(i)**2+unitly(i)**2))
       endif

       hyp(i) = sqrt( (unitlx(i)*ecc_vec_y(i)-unitly(i)*ecc_vec_x(i))**2+(unitlx(i)+unitly(i))**2&
                        *(ecc_vec_x(i)**2+ecc_vec_z(i)**2))&
                        /sqrt(dot_product(ecc_vec(1:3,i),ecc_vec(1:3,i)))

       if (ecc_vec_z(i)>=0) then
        omega_(i) = acos(dot_product(node_vec(1:3,i), ecc_vec(1:3,i)/sqrt(dot_product(ecc_vec(1:3,i),ecc_vec(1:3,i)))) / hyp(i))
       else ; omega_(i) = acos(2*pi - dot_product(node_vec(1:3,i), ecc_vec(1:3,i)/sqrt(dot_product(ecc_vec(1:3,i),ecc_vec(1:3,i))))&
                                / hyp(i))
       endif
    endif
 enddo
 ...
 if (rotate) then
    if (nptmass /= 0) then
       call get_total_angular_momentum(L_tot=L_tot,&
                                       xyzh=xyzh,vxyz=vxyz,npart=npart,&
                                       xyzmh_ptmass=xyzmh_ptmass,vxyz_ptmass=vxyz_ptmass,npart_ptmass=nptmass,&
                                       only_sinks=ref_is_only_sinks,sinks_index=sinks_L)
    else
       call get_total_angular_momentum(xyzh,vxyz,npart,L_tot)
    endif
...
end subroutine bindisc_analysis
